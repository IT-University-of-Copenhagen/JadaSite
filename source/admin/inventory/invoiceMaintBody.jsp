...
<c:set var="invoiceMaintForm" scope="request" value="${invoiceMaintForm}"/>
<c:set var="orderMaintForm" scope="request" value="${invoiceMaintForm}"/>

<script language="JavaScript">
function submitForm(methodType) {
    document.invoiceMaintForm.action = '/<c:out value='${adminBean.contextPath}'/>/admin/inventory/invoiceMaint.do';
    document.invoiceMaintForm.process.value = methodType;
    document.invoiceMaintForm.submit();
    return false;
}
function submitCreditForm(methodType) {
    document.invoiceMaintForm.action = '/<c:out value='${adminBean.contextPath}'/>/admin/inventory/creditMaint.do';
    document.invoiceMaintForm.process.value = methodType;
    document.invoiceMaintForm.submit();
    return false;
}
function submitBackForm(type) {
    document.invoiceMaintForm.action = "/<c:out value='${adminBean.contextPath}'/>/admin/inventory/orderListing.do";
    document.invoiceMaintForm.process.value = type;
    document.invoiceMaintForm.submit();
    return false;
}
...
<html:form action="/admin/inventory/invoiceMaint.do" method="post">
    ...
    <td valign="top">
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="jc_panel_table_header">
        <tr>
          <td>
	        <c:choose>
	          <c:when test="${invoiceMaintForm.newInvoice}">
	            New Invoice
	          </c:when>
	          <c:otherwise> 
	            Invoice Order - ${invoiceMaintForm.invoiceHeader.invoiceNum}
	          </c:otherwise>
	        </c:choose>
          </td>
          <td align="right"> 
            <table width="0%" border="0" cellspacing="0" cellpadding="0">
              <tr> 
                <td>
                  <c:if test="${invoiceMaintForm.allowVoid}">
                  <html:submit property="submitButton" value="Void Payment" styleClass="jc_submit_button" onclick="return submitForm('voidOrder');"/>&nbsp;
                  </c:if>
                  <c:if test="${invoiceMaintForm.allowCapture}">
                  <html:submit property="submitButton" value="Capture Payment" styleClass="jc_submit_button" onclick="return submitForm('capture');"/>&nbsp;
                  </c:if>
                  <c:if test="${invoiceMaintForm.allowCredit}">
                  <html:submit property="submitButton" value="Credit" styleClass="jc_submit_button" onclick="return submitCreditForm('create');"/>&nbsp;
                  </c:if>
                  <c:if test="${invoiceMaintForm.editable}">
      ...
      <logic:messagesPresent property="invoice" message="true">
        <html:messages property="invoice" id="errorText" message="true"> 
        ...
        <tr>
          <td>
            <table width="100%" border="0" cellpadding="3" class="order">
	          ...
	          <c:forEach var="invoiceDetail" items="${invoiceMaintForm.invoiceDetails}">
	          <c:choose> 
	          ...
	          <tr class="${className}">
                <td valign="top">
                  <html:hidden indexed="true" name="invoiceDetail" property="orderItemDetailId"/>
                  <html:hidden indexed="true" name="invoiceDetail" property="invoiceDetailId"/>
                  <html:hidden indexed="true" name="invoiceDetail" property="itemId"/>
                  <html:hidden indexed="true" name="invoiceDetail" property="itemSkuCd"/>
                  <html:hidden indexed="true" name="invoiceDetail" property="itemShortDesc"/>
                  <html:hidden indexed="true" name="invoiceDetail" property="itemTierQty"/>
                  <html:hidden indexed="true" name="invoiceDetail" property="itemTierPrice"/>
                  <html:hidden indexed="true" name="invoiceDetail" property="itemOrderQty"/>
                  <html:hidden indexed="true" name="invoiceDetail" property="itemInvoiceQty"/>
                  <html:hidden indexed="true" name="invoiceDetail" property="itemCreditQty"/>
                  <html:hidden indexed="true" name="invoiceDetail" property="itemShipQty"/>
                  <html:hidden indexed="true" name="invoiceDetail" property="itemInvoiceAmount"/>
                  ${invoiceDetail.itemSkuCd}
                </td>
                <td>
                  ${invoiceDetail.itemShortDesc}<br>
                  <c:forEach var="orderItemAttribute" items="${invoiceDetail.orderItemAttributes}">
                  ${orderItemAttribute.customAttribDesc} - ${orderItemAttribute.customAttribValue}<br>
                  </c:forEach>
                </td>
                <td valign="top">${invoiceDetail.itemTierPrice}</td>
                <td valign="top"><div align="center">${invoiceDetail.itemOrderQty}</div></td>
                <td valign="top"><div align="center">${invoiceDetail.itemInvoiceQty}</div></td>
                <td valign="top"><div align="center">${invoiceDetail.itemCreditQty}</div></td>
                <td valign="top"><div align="center">${invoiceDetail.itemShipQty}</div></td>
                <td valign="top">
                  <div align="center">
                  <c:choose>
	                  <c:when test="${invoiceMaintForm.editable}">
	                  <html:text indexed="true" name="invoiceDetail" styleClass="jc_input_control" size="5" property="inputInvoiceQty"/>
	                  </c:when>
	                  <c:otherwise>
	                  <html:hidden indexed="true" name="invoiceDetail" property="inputInvoiceQty"/>
	                  ${invoiceDetail.inputInvoiceQty}
	                  </c:otherwise>
                  </c:choose>
                  <c:if test="${!empty invoiceDetail.inputInvoiceQtyError}">
                  <br>
                  <span class="jc_input_error">
                  ${invoiceDetail.inputInvoiceQtyError}
                  </span>
                  </c:if>
                  </div>
                </td>
                <td valign="top"><div align="right">${invoiceDetail.itemInvoiceAmount}</div></td>
              </tr>
	          </c:forEach>
	          <tr class="order">
                <td></td>
                ...
                <td></td>
                <td><div align="center">${invoiceMaintForm.invoiceHeader.itemOrderQty}</div></td>
                <td><div align="center">${invoiceMaintForm.invoiceHeader.itemInvoiceQty}</div></td>
                <td><div align="center">${invoiceMaintForm.invoiceHeader.itemCreditQty}</div></td>
                <td><div align="center">${invoiceMaintForm.invoiceHeader.itemShipQty}</div></td>
                <td><div align="center">${invoiceMaintForm.invoiceHeader.inputInvoiceQty}</div></td>
                <td><div align="right">${invoiceMaintForm.invoiceHeader.itemInvoiceAmount}</div></td>
              </tr>
            </table>
          </td>
        </tr>
	  ...
	  <table width="100%" border="0" cellpadding="3">
        <tr>
          <td width="100%" valign="top">
            <c:if test="${!invoiceMaintForm.newInvoice}">
	        <table width="100%" border="0" cellpadding="3" class="order">
              <tr class="orderHeader">
              ...
              <tr>
                <td width="100%">
                  <html:textarea name="invoiceMaintForm" property="orderTrackingMessage" styleClass="tableContent"/><br>
                  <html:checkbox name="invoiceMaintForm" property="orderTrackingInternal" styleClass="tableContent">Internal view only</html:checkbox>
                </td>
            ...
            <table width="100%" border="0" cellpadding="3" class="order">
              ...
              <c:set var="single">true</c:set>
	          <c:forEach var="orderTracking" items="${invoiceMaintForm.invoiceHeader.orderTrackings}">
	          <c:choose> 
	            <c:when test="${single == 'true'}">
	              <c:set var="single">false</c:set>
	              <c:set var="className">order</c:set>
	      ...
	      <c:choose>
            ...
            <table width="300" border="0">
              <tr>
                ...
                <td align="right">
                  <html:text name="invoiceMaintForm" styleClass="jc_input_control" size="5" property="inputShippingTotal"/>
                  <span class="jc_input_error">
            ...
            <c:otherwise>
            <html:hidden name="invoiceMaintForm" property="inputShippingTotal"/>
            </c:otherwise>
          </c:choose>
            <div class="jc_detail_panel_header">
            <table width="300" border="0" cellspacing="0" cellpadding="0">
            ...
            <table width="300" border="0">
              <tr>
                ...
                <td align="right">${invoiceMaintForm.invoiceHeader.itemInvoiceAmount}</td>
              </tr>
              <tr>
                <td><span class="jc_input_label">Shipping</span></td>
                <td align="right">${invoiceMaintForm.invoiceHeader.shippingTotal}</td>
              </tr>
              <c:forEach var="invoiceTax" items="${invoiceMaintForm.invoiceTaxes}">
              <tr>
                <td><span class="jc_input_label">${invoiceTax.taxName}</span></td>
                <td align="right">${invoiceTax.taxAmount}</td>
              </tr>
              </c:forEach>
              <tr>
                <td><span class="jc_input_label">Total</span></td>
                <td align="right">${invoiceMaintForm.invoiceHeader.invoiceTotal}</td>
              </tr>
            </table>
            </div>
	      </td>
