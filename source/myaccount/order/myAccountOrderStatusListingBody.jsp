...
<div id="my-account-container">
  ...
  <div id="my-account-body-container">
    <html:form method="post" action="/myaccount/order/myAccountOrderStatusListing.do?process=update">
    <html:hidden property="process" value=""/>
    <div id="my-account-header-container"><lang:contentMessage key="content.text.myaccount.menu.orderStatus"/></div>
      ...
      <table border="0" cellspacing="0" cellpadding="0" id="my-account-order-listing-table">
        <tr> 
          <td colspan="5" align="right">
            ...
            <a class="my-account-order-status-nav-link" href="/${contentBean.contextPath}/myaccount/order/myAccountOrderStatusListing.do?process=list&prefix=${contentBean.siteDomain.siteDomainPrefix}&langName=${contentBean.contentSessionKey.langName}&srPageNo=<c:out value="${form.pageNo - 1}"/>">&lt;</a>
            </c:if>
            <c:forEach begin="${form.startPage}" end="${form.endPage}" var="index">
              <c:choose>
                ...
                <c:otherwise>
                  <a class="my-account-order-status-nav-link" href="/${contentBean.contextPath}/myaccount/order/myAccountOrderStatusListing.do?process=list&prefix=${contentBean.siteDomain.siteDomainPrefix}&langName=${contentBean.contentSessionKey.langName}&srPageNo=<c:out value='${index}'/>">${index}</a>
                </c:otherwise>
              </c:choose>
            </c:forEach>
            &nbsp;
        ...
        <tr> 
          ...
          <td nowrap align="right"><span class="my-account-order-listing-table-title"><lang:contentMessage value="Total"/></span></td>
        </tr>
        ...
        <tr> 
          ...
          <td nowrap>
            <a class="my-account-order-status-link" href="/<c:out value='${contentBean.contextPath}'/>/myaccount/order/myAccountOrderStatus.do?process=start&prefix=${contentBean.siteDomain.siteDomainPrefix}&langName=${contentBean.contentSessionKey.langName}&orderHeaderId=<c:out value='${order.orderHeaderId}'/>">
              ${order.orderNum}
            </a>
          </td>
